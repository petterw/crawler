...
class Crawler:
	...
	def __init__(self, 
		...
		crawl_domains = [], # optionally, restrict the robot to these domains
		concurrent_fetchers = 1): # The number of documents that may be scheduled for fetching
		# at the same time. Don't set this above the number of celery worker processes. """
	...
	def add_url(self, url_tuple):
		...
		if parts[0] in self.schemes and (len(self.crawl_domains) == 0 or (parts[1] in self.crawl_domains)):
			...
			if self.results.get(url_tuple[0]) == None:
				...
				document =  Document(url_tuple[0], self.results[url_tuple[1]], self.domains[dname])
	...
	def crawl(self, save_frequency = None, termination_checker = lambda c: False):
		...
		while not self.done:
			...
			""" Consider moving crawled documents from in progress queue to url extraction queue,
				or remove them from crawling altogether if we were blocked by robots.txt 
			"""
...
class Document:
	...
	def retrieve(self):
		""" Start retrieval of document.
			...
			time may be executed later. But on average (not under massive load), it will be obeyed.
			
			I'm aware that this is a limitation of the crawler, and should be fixed.
		...
		if self.domain.allows_crawling(self.url):
			...
			self.task = fetcher.fetch_document.apply_async(args = [self.url, self.domain.crawler.useragent_string], eta = self.crawl_time)
			
		else:
			# Blocked by robots.txt
			self.blocked = True
			#raise CrawlingError("Disallowed by robots.txt (url = " + self.url+ ")")
